...
function FooterCol({ title, items }: { title: string; items: { label: string; cb?: () => void }[] }) {
  return (
    <div>
      <h4 style={{ fontSize: 10, fontWeight: 500, letterSpacing: 3, textTransform: "uppercase", color: "var(--text-primary)", marginBottom: 22 }}>
        {title}
      </h4>
        ...
        {items.map((it) => (
          <li key={it.label}>
            <a
              ...
              style={{ fontSize: 13, fontWeight: 300, color: "var(--text-secondary)", textDecoration: "none", transition: "color 0.2s" }}
      ...
      <div style={{ maxWidth: 1440, margin: "0 auto" }}>
        <div style={{ display: "grid", gridTemplateColumns: "var(--footer-cols)", gap: "var(--footer-gap)", marginBottom: 64 }}>
          <div>
            ...
            <p style={{ fontSize: 12, fontWeight: 300, color: "var(--text-secondary)", lineHeight: 1.7, marginTop: 20, maxWidth: 280 }}>
              {TEXTS.footerTagline}
            </p>
          </div>
          ...
          <FooterCol
            ...
            items={[
              ...
              { label: "Акції", cb: () => onNavClick({ id: "aktsii", label: "Акції", scrollTo: "promos" }) },
            ]}
          />
          ...
          <div>
            ...
            <div style={{ fontSize: 14, fontWeight: 300, color: "var(--text-primary)", marginBottom: 10 }}>{CONTACTS.phone}</div>
            <div style={{ fontSize: 12, fontWeight: 300, color: "var(--text-secondary)", lineHeight: 1.7, marginBottom: 20 }}>
        ...
        <div style={{ borderTop: "1px solid var(--border)", paddingTop: 32, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
          <div style={{ fontSize: 10, fontWeight: 300, letterSpacing: 2, color: "var(--text-secondary)" }}>
            © 2025 SUSHI SYNDICATE · ТУЛЬЧИН, ВІННИЦЬКА ОБЛАСТЬ
          </div>
          <div style={{ fontFamily: "var(--font-display)", fontStyle: "italic", fontSize: 14, color: "var(--text-secondary)" }}>
            {TEXTS.tagline}
          </div>
        </div>
      </div>
